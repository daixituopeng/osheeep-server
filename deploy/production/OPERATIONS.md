...
订阅消息默认必须保持 `OSHEEEP_WECHAT_SUBSCRIPTION_ENABLED=false`。只有在公众平台为同一小程序取得三条不同标题的正式模板，并逐项抄录模板 ID、平台标题及 subject/time/note 字段 key 后，才可在发布窗口改为 `true`；生产状态使用 `OSHEEEP_WECHAT_SUBSCRIPTION_STATE=formal`。模板 ID 和字段 key 不得写入 Git、聊天、普通日志或截图。

开启时应用会在启动阶段执行硬门禁：三套配置必须完整，三个模板 ID 和三个平台标题分别互不重复，每套的三个字段 key 必须合法且互不重复，运行状态只能是 `developer`、`trial` 或 `formal`。任一条件不满足，服务必须启动失败；不得删除门禁、填入占位值或只启用部分场景来绕过失败。保持关闭时，订阅配置 API 返回空动作，调度器不调用微信，但站内消息和业务功能正常工作。

修改正式值前后都应保留环境文件备份。重启后先确认健康检查和 V10 Flyway 成功，再由两个真实账号分别验证 `PARTNER_JOINED`、`MENU_CHANGED`、`MENU_COMPLETED` 的显式授权、一次性发送、拒绝不影响业务及站内消息保留；没有这些证据时不得把订阅消息写成已上线。
