...
Each recipe contains `id`, `name`, nullable `imagePath`, `category`, `flavor`, and `estimatedMinutes`, plus:
...
Discovery includes published system recipes and valid published household recipes owned by the authenticated user's active household. A household recipe may have no ingredient rows and no image; in that case `ingredients` is empty and `imagePath` is `null`. It is omitted if its publishable aggregate has become invalid, if a persisted ingredient is not visible to the recipe, if it does not have exactly one valid active default method, or if a selected image can no longer provide an approved self-hosted list URL. Drafts, archived recipes, and recipes from other households are never returned.
...
`householdPreference` is one of `BOTH_LIKE`, `SOME_LIKE`, `NEUTRAL`, `MIXED`, `SOME_DISLIKE`, or `BOTH_DISLIKE`. It exposes no user or membership ID. `myPreference` defaults to `NEUTRAL` with `myVersion: 0` before this member has written a choice. Discovery loads all visible recipe preferences in one batch, but a default request with no inventory filter does not use inventory or preference ranking. It orders recipes by lightweight dish kind (meat, vegetable, then unclassified), current-household recipes before system recipes within each kind, ascending recipe name, and finally ascending recipe ID. Dish-kind compatibility follows the client rule: explicit `荤菜` / `素菜` values are used directly; legacy category/flavor text containing only `素` maps to vegetable, while text containing `荤` or `肉` maps to meat.

When `onlyCookable=true`, `includeIngredientIds` is non-empty, or `excludeIngredientIds` is non-empty, the request explicitly opts into inventory discovery. That compatibility path keeps the original ordering by match status (`AVAILABLE`, `UNKNOWN_QUANTITY`, `MISSING`), descending `matchPercent`, household preference (`BOTH_LIKE`, `SOME_LIKE`, `NEUTRAL`, `MIXED`, `SOME_DISLIKE`, `BOTH_DISLIKE`), ascending `estimatedMinutes` with unknown duration last, and finally ascending recipe ID. Match fields remain present on both paths.
...
On 2026-08-31, the current V16 line passed the guarded local MySQL 8 suites in a loopback-only, no-volume Docker container: `DinnerCookingV15MySqlIT` passed 1/1 for fresh V1→V16, production-shaped V13→V16, concurrency and rollback coverage; `DinnerCustomRecipeMySqlIT` passed 7/7. Its lightweight vertical case creates and publishes a household recipe without image or ingredient rows, discovers it, selects it, transitions the draft menu directly to cooking, completes the frozen dish, creates and reads the immutable record, verifies no standalone confirm action was written, and confirms an unrelated household inventory row kept the exact same quantity, unit, and version. This run also caught and fixed the custom cooking-row query mapping for `method_steps AS method_steps_json`. It is backend HTTP + MySQL evidence only; moderation was mocked and no real WeChat API or device was used.
...
export OSHEEEP_DB_NAME=osheeep_it_v16
export OSHEEEP_DB_TEST_NAME=osheeep_it_v16
...
mvn test -Dtest=DinnerCookingV15MySqlIT -Dspring.profiles.active=local
mvn test -Dtest=DinnerCustomRecipeMySqlIT -Dspring.profiles.active=local
mvn test
```

The raw `OSHEEEP_DB_NAME` and `OSHEEEP_DB_TEST_NAME` environment values must be identical; JVM `-D` overrides do not satisfy that trust anchor. For the migration IT, the base catalog must match `[A-Za-z0-9_]+`, be at most 15 characters, and be reached through `localhost`, `127.0.0.1`, or loopback IPv6. The opt-in value must be exactly `true`, `SELECT DATABASE()` must equal the base name, and the MySQL account must be able to create and drop the three run-specific ephemeral catalogs. The harness deletes those generated catalogs in `close()`. For the business IT, the `local` profile's datasource URL must resolve to the same base catalog, no separate `spring.flyway.url` may be set, and the base catalog must remain disposable because Flyway V1-V16 and the business fixtures write to it. The Redis, RabbitMQ, WeChat, and JWT values above only satisfy required local-profile configuration; the integration tests do not treat them as evidence of reachable external services, and moderation is replaced with a local Mockito `PASS` result.

The integration test replaces `DinnerTextSafetyGateway` with a Mockito `PASS` result. It verifies that publication looks up the seeded active user's `openid`, passes that identity and the normalized recipe text to the gateway, and then completes the publication transaction. It does not call the real WeChat `msgSecCheck` service and is not evidence of external WeChat availability or acceptance.
...
Creating a draft returns version `1`. Every successful basic-info, ingredient, default-method, complete-method-set, or image write increments the aggregate version exactly once. Publication also increments once. Ingredient and image writes are optional: a minimal household recipe can progress through basic info, one internal default method, and publication without either route. Every write must supply the exact current version. A stale version returns HTTP 409, `DINNER_RECIPE_VERSION_CONFLICT`, with message `Dinner recipe was updated elsewhere`; the server does not replay the write.
...
Each merged menu dish contains `recipeId`, `name`, nullable `imagePath`, `category`, `flavor`, `estimatedMinutes`, `source`, `selectedBy`, `scope`, `recipeVersion`, nullable `method`, `methodChoices`, and `methodConflict`. A method choice is `{method,selectedBy}` and preserves the privacy-safe actors who selected that active method. `method` is the single effective `{id,name,cookingStyle}` summary when all selectors agree or after confirmation resolves a conflict; `methodConflict` is true when the two members selected different active methods. A system recipe is saved and returned with `scope: "SYSTEM"`, `recipeVersion: 1`, `method: null`, an empty `methodChoices` array and no conflict. A household recipe is saved with its current positive aggregate version and an explicitly selected active method, or the active default method for a legacy request. A household recipe without an image remains selectable and returns `imagePath: null`.
...
The chosen method must be one of the methods actually selected for that recipe. Missing resolutions return HTTP 409 `DINNER_MENU_METHOD_RESOLUTION_REQUIRED`; duplicates, extra recipes or unselected methods return HTTP 400 `DINNER_MENU_METHOD_RESOLUTION_INVALID`. The service applies all exact resolutions and confirmation in one transaction. Confirming an empty menu returns `DINNER_MENU_EMPTY`. Updating a completed menu returns `DINNER_MENU_COMPLETED`; selection or confirmation writes after cooking starts return `DINNER_MENU_COOKING`. This explicit confirm route remains available for already released clients.

### Actual cooking session (V15)

Starting cooking accepts either `DRAFT` or `CONFIRMED`. For a draft, the server resolves every method conflict inside the start transaction: it prefers the starting member's selected method and otherwise uses the lowest selected method ID. It updates all saved selections for that recipe, records confirmation metadata, freezes the planned dishes, and transitions directly to `COOKING`. Only one `START_COOKING` action is written and the menu version increments exactly once; no separately committed `CONFIRMED` window is exposed. A confirmed menu follows the same existing freeze path without another confirmation write. Empty drafts return `DINNER_MENU_EMPTY`.

The frozen session includes the display summary, selected recipe version, servings, selected method name/style/estimated time and ordered steps, ingredients, selectors, and nullable image path. A minimal household recipe freezes `ingredients: []` and `imagePath: null`. Later live recipe edits, archive operations, method changes, ingredient changes, or image metadata changes cannot alter this cooking session. The start request is replay-safe by its UUID v4 key; replay returns the already frozen `COOKING` or `COMPLETED` session without re-resolving methods or writing another action.
...
Flyway migration `V16__allow_empty_cooking_ingredients.sql` replaces only the V15 cooking-snapshot shape check. It keeps positive estimated time, non-negative sort order, array-typed ingredient JSON, and non-empty selectors, while allowing the ingredient array itself to be empty for lightweight household recipes. V1–V15 remain unchanged.

The explicitly selected `DinnerCookingV15MySqlIT` is the guarded MySQL 8 acceptance suite for this migration line and transaction boundary. It now verifies fresh-to-V16 and production-shaped V13-to-V16 migration, V13 snapshot `origin` backfill, V15/V16 constraints, two ACTIVE members racing add/completion writes at one version, concurrent final completion creating exactly one record with replay after conflict, rollback after a forced real snapshot-insert failure, and legacy `CONFIRMED` completion without cooking rows. It requires raw `OSHEEEP_DB_NAME == OSHEEEP_DB_TEST_NAME`, `OSHEEEP_ALLOW_EPHEMERAL_DATABASES=true`, and a loopback MySQL URL. It then creates only UUID-scoped child catalogs through `DinnerEphemeralCatalogHarness`, closes the Spring pool before cleanup, drops all generated catalogs, and verifies their absence. Never source the development or production `.env.local` for this test. Ordinary `mvn test` does not select `*MySqlIT`; run it explicitly with `mvn test -Dtest=DinnerCookingV15MySqlIT` after supplying a dedicated disposable loopback catalog.
